...
function Do-Post([string]$path, [hashtable]$body, [int]$timeoutSec=90) {
  ...
  try {
    $r = Invoke-WebRequest -Uri ($base+$path) -Method POST -Headers $auth -Body $json -TimeoutSec 30 -UseBasicParsing -ErrorAction Stop
    $j = $r.Content | ConvertFrom-Json
    if ($path -eq '/terminal-exec' -and $j.runId) {
      $deadline = (Get-Date).AddSeconds($timeoutSec)
      while ((Get-Date) -lt $deadline) {
        Start-Sleep -Milliseconds 500
        $rr = Invoke-WebRequest -Uri ($base+'/terminal-result?id='+$j.runId) -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
        $q = $rr.Content | ConvertFrom-Json
        if ($q.done) { return @{ status = 200; body = @{ exitCode = $q.exitCode; timedOut = $q.timedOut; output = $q.output; error = $q.error; session = $body.session } } }
      }
      return @{ status = 200; body = @{ exitCode = $null; timedOut = $true; output = ''; error = 'wait timeout'; session = $body.session } }
    }
...
Add-Result 'B' 'inline whoami SYSTEM' ($b.status -eq 200 -and $b.body.exitCode -eq 0 -and ($b.body.output -match 'nt authority.system')) ('exit=' + $b.body.exitCode)
$c = Do-Post '/terminal-exec' @{ mode='file'; session='system'; file='C:\ghrdp\ghrdp-pub2.ps1'; timeout=10000 }
Add-Result 'C' 'file mode ghrdp-pub2.ps1' ($c.status -eq 200) ('exit=' + $c.body.exitCode)
...
Add-Result 'D' 'upload 50-line script' ($d.status -eq 200 -and $d.body.exitCode -eq 0 -and ($d.body.output -match 'line-50')) ('exit=' + $d.body.exitCode)
$e = Do-Post '/terminal-exec' @{ mode='inline'; session='interactive'; cmd='whoami'; timeout=30000 }
Add-Result 'E' 'interactive whoami' ($e.status -eq 200 -and $e.body.session -eq 'interactive') ('exit=' + $e.body.exitCode)
...
Add-Result 'G' 'timeout kills process' ($g.status -eq 200 -and $g.body.timedOut -eq $true) ('timedOut=' + $g.body.timedOut)
...
$iOk = $true; $iErr = ''
for ($i=1; $i -le 6; $i++) {
  $r2 = Do-Post '/terminal-exec' @{ mode='inline'; session='system'; cmd=('"loop-' + $i + '"'); timeout=5000 }
  if ($r2.status -ne 200 -or $r2.body.exitCode -ne 0) { $iOk = $false; $iErr = ('#' + $i + ' status=' + $r2.status); break }
}
Add-Result 'I' 'rapid sequential no file-lock' $iOk $iErr
try { $j2 = Invoke-WebRequest -Uri ($base+'/terminal?token='+$token) -UseBasicParsing -TimeoutSec 10
  Add-Result 'J' 'reduced-transparency fallback CSS' ($j2.Content -match 'prefers-reduced-transparency') '' }
catch { Add-Result 'J' 'reduced-transparency fallback CSS' $false $_.Exception.Message }
try { Add-Result 'K' '@supports backdrop fallback CSS' ($j2.Content -match '@supports not \(backdrop-filter') '' }
catch { Add-Result 'K' '@supports backdrop fallback CSS' $false 'no fetch' }
try { $r3 = Invoke-WebRequest -Uri ($base+'/terminal-exec') -Method POST -Headers @{ Authorization='Bearer wrong-token'; 'Content-Type'='application/json' } -Body '{}' -TimeoutSec 10 -UseBasicParsing -ErrorAction Stop
  Add-Result 'L' '401 on bad token' $false ('unexpected status=' + $r3.StatusCode) }
catch { $sc = 0; try { $sc = [int]$_.Exception.Response.StatusCode.value__ } catch { }; Add-Result 'L' '401 on bad token' ($sc -eq 401) ('status=' + $sc) }
