...
- F9i fixes are CI-verified only: user must manually dispatch the NEW main workflow (VNC_PASS already set), watch the web-desktop + self-test steps go green, and click WEB DESKTOP; Proof C (real VNC login + visible desktop) still required.

## Anchors (re-derive by search)
- main.yml: MagicDNS gates ~:303/:355/:835; F9g retry ~:858; Cleanup ~:2676. Web-desktop step ~:1029 (F9h guard ~:1063, belt ~:1087, F9i websockify dual-log Start-Process + fail-closed exits inside); self-test ~:1285. ghrdp-server.ps1: native-status ~:381 (webdeskReason/Detail/vncPassAdminUrl fields there); 404 guard ~:238. payloads/main.rs: api_config secret-strip ~:283. helper: mutex ~:105. MIGRATION.md: web desktop sec 1.10 ~:294 (F9h/U5c/F9i bullets).
